...
PCF8574 rotaryExpander; // rotary encoder **BUTTONS**
PCF8574 footExpander; // foot switch buttons
...
PCF8574 LeftPhaseRelays;
PCF8574 RightPhaseRelays;
PCF8574 ReturnRelayExpander;
...
void setup() {
  ...
  //EXPANDERS
  rotaryExpander  = PCF8574(ROTARY_BUTTONS_ADDRESS); 
  footExpander = PCF8574(FOOTSWITCH_ADDRESS);
  ...
  initializeRelays();
	...
	pinMode(rotaryExpander, 0, INPUT_PULLUP); // these pins are default Serial, needs changed most likely
  ...
  Serial.println("From after Matrix Wipes");
...
//-----------------------------------LOOP-------------------------------------
void loop() {
  Serial.println("prints working from loop");
  delay(1000);
...
void doFoot(){   
        FootFlag = false;
        int footID = footHextoID(footExpander.read()); //when 2 are pressed and when released sends original hex again aka the other one pressed
